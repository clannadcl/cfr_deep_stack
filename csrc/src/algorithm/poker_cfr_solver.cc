...
std::shared_ptr<game::poker::SubgameSetup>
ValidateSetup(std::shared_ptr<game::poker::SubgameSetup> setup) {
  if (setup == nullptr) {
...
float DcfrAverageStrategyDiscount(int iteration, float exponent) {
  ...
  const double ratio =
      static_cast<double>(iteration) / static_cast<double>(iteration + 1);
  return static_cast<float>(std::pow(ratio, static_cast<double>(exponent)));
}

std::vector<EvRawHandEntry>
BuildEvRawHandEntries(const game::poker::GameBasic &game_basic,
...
std::vector<float>
ComputeEvValidMass(const std::vector<EvRawHandEntry> &hands,
                   const game::poker::IsomorphicMapping &mapping,
  ...
  std::vector<float> valid_mass(static_cast<std::size_t>(mapping.NumIsoHands()),
                                0.0f);
  for (const EvRawHandEntry &hand : hands) {
    valid_mass[static_cast<std::size_t>(hand.iso_index)] += static_cast<float>(
...
void ValidateAdjacentBoardTransition(
  ...
  if (child_cards != parent_cards + 1) {
    throw std::invalid_argument(
        "Iso transition child board must be next street");
  }
...
PokerCfrSolver::Args::Args(std::shared_ptr<game::poker::SubgameSetup> setup,
    ...
    : setup(std::move(setup)), num_threads(num_threads),
      ...
      target_exploitability(target_exploitability), use_dcfr(use_dcfr),
...
public:
  ...
  template <typename Fn> void ParallelFor(std::size_t count, Fn fn) {
...
PokerCfrSolver::PokerCfrSolver(const Args &args)
    : setup_(ValidateSetup(args.setup)), tree_(setup_),
...
std::vector<float> PokerCfrSolver::AverageStrategyData(float epsilon) const {
  ...
  for (const game::poker::PokerTreeNode &node : tree_.Nodes()) {
    ...
    for (int hand = 0; hand < layout.num_hands; ++hand) {
      ...
      for (int action = 0; action < layout.num_actions; ++action) {
        average[static_cast<std::size_t>(layout.sum_strategy_offset +
                                         action * layout.num_hands + hand)] =
            (storage_.SumStrategyAt(node.node_id, action, hand) + epsilon) /
            denominator;
      }
    }
...
float PokerCfrSolver::AverageStrategyAt(int node_id, int action_index,
  ...
  if (action_index < 0 || action_index >= layout.num_actions) {
    throw std::invalid_argument(
        "Average strategy action index is out of range");
  }
  ...
  for (int action = 0; action < layout.num_actions; ++action) {
    denominator +=
        storage_.SumStrategyAt(node_id, action, hand_index) + epsilon;
  }
  return (storage_.SumStrategyAt(node_id, action_index, hand_index) + epsilon) /
         denominator;
}
...
const game::poker::IsomorphicMapping &
PokerCfrSolver::MappingForNode(int node_id) const {
  tree_.Node(node_id);
  const auto index = static_cast<std::size_t>(node_id);
  if (index >= node_mappings_.size() || node_mappings_[index] == nullptr) {
    throw std::invalid_argument("Poker CFR node mapping is unavailable");
  }
...
const IsoTransition &PokerCfrSolver::ChanceTransition(int child_node_id) const {
  if (child_node_id < 0 ||
      child_node_id >=
...
void PokerCfrSolver::BuildNodeCaches() {
  ...
  for (const game::poker::PokerTreeNode &node : tree_.Nodes()) {
    ...
    for (int player = 0; player < game::poker::GameBasic::kNumPlayers;
         ++player) {
      active_iso_hands_by_node_player_[static_cast<std::size_t>(
          node.node_id)][static_cast<std::size_t>(player)] =
    ...
    for (int child_index = 0; child_index < node.num_children; ++child_index) {
      ...
      if (tree_.ChildNodeIdAt(node.node_id, child_index) != expected_child_id) {
...
std::uint64_t PokerCfrSolver::EstimateNodeTraversalWork(
  ...
  if (node.node_state->ActorPlayer() == game::poker::NodeState::kChancePlayer) {
  ...
  for (int child_index = 0; child_index < child_cache.num_children;
    ...
    for (int player = 0; player < game::poker::GameBasic::kNumPlayers;
      ...
      for (const IsoTransitionEdge &edge : transition.edges) {
        child_reach[edge.child_iso] +=
            parent_reach[edge.parent_iso] * edge.weight;
      }
    }
  }
}
...
void PokerCfrSolver::ComputeTerminalCfvs(int player) {
  ...
  thread_pool_->ParallelFor(fold_items.size(), kFoldTerminalGrain,
                            [&](std::size_t index) {
                              const TerminalBatchItem &item = fold_items[index];
  ...
  for (const TerminalBatchItems &runout_items : runout_batches) {
    ...
    thread_pool_->ParallelFor(num_chunks, [&](std::size_t chunk_index) {
      ...
      terminal_cfv_calculator_.CalculateRunoutShowdownBatch(runout_items, begin,
                                                            end);
    });
  }
}
...
void PokerCfrSolver::BackwardChanceNode(const game::poker::PokerTreeNode &node,
                                        int player) {
  ...
  for (int child_index = 0; child_index < child_cache.num_children;
    ...
    for (const IsoTransitionEdge &edge : transition.edges) {
      parent_cfv[edge.parent_iso] +=
          child_cfv[edge.child_iso] * edge.weight * transition.chance_prob;
    }
  }
}

void PokerCfrSolver::BackwardPlayerNode(const game::poker::PokerTreeNode &node,
                                        int hero_player) {
  const int actor = node.node_state->ActorPlayer();
  const NodeCfrLayout &layout = storage_.Layout(node.node_id);
  const NodeChildCache &child_cache =
  ...
  for (int action = 0; action < layout.num_actions; ++action) {
    ...
    float *action_strategy =
        mutable_strategy + static_cast<std::size_t>(action * layout.num_hands);
    for (int hand : hero_active_hands) {
...
void PokerCfrSolver::BackwardAveragePlayerNode(
  ...
  for (int player = 0; player < game::poker::GameBasic::kNumPlayers; ++player) {
    for (int hand = 0; hand < layout.num_hands; ++hand) {
      ...
      for (int action = 0; action < layout.num_actions; ++action) {
        const int child_id = child_cache.first_child_id + action;
        const double child_value = storage_.CfvAt(child_id, player, hand);
        value += player == actor
...
std::vector<float> PokerCfrSolver::ReachVector(int node_id, int player) const {
  std::vector<float> values(
      static_cast<std::size_t>(storage_.NumHands(node_id)), 0.0f);
  for (int hand = 0; hand < storage_.NumHands(node_id); ++hand) {
